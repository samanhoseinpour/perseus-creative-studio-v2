...
/**
 * The foot of every protected page — a COLOPHON, which is a sentence, not a
 * bar. Two lines: who made the thing, then what build of it you are looking at.
 *
 * It has been four things. A `border-t` with a lone right-aligned link; then a
 ...
 * other; then this line's first form, which was bare type and correct but said
 * nothing. Each of the first two added an OBJECT, and both were rejected on
 * sight. Four things were wrong with them and all four still constrain what
 * may go here:
 *
 ...
 *
 * So the second line is unchanged and the interest is paid in TYPE, not in a
 * new object: a credit line above it, one tonal step brighter, with the name as
 * the only link. Nothing here is drawn, so nothing here can break at 2100px.
 *
 * LEFT-ALIGNED to the content column, separated by whitespace alone. Left rather
 * than centred is structural, not taste — a centred caption has no ends to drift
 * but its own left edge then travels from ~350px under `narrow` to ~960px under
 * `table`, abandoning the one vertical line the eye tracks down the page; and
 * small centred grey type at the dead bottom of a page is the copyright-line
 * shape this dashboard must not wear.
 *
 * THE GITHUB LINK OPENS IN A NEW TAB, and that is load-bearing rather than
 * politeness: the dashboard installs to a Home Screen, and a same-tab out-link
 * navigates an installed app clean out of /admin with no address bar to come
 * back from. Every deliberate out-link in this tree carries target="_blank".
 *
 ...
 *
 * Ink only, three tonal steps — credit at foreground/70, muted label, ink
 * numeral, foreground/25 middot. The theme has no chroma, so the unread state is
 * carried typographically too: the link goes from muted to ink and its underline
 * is already drawn.
 *
...
export default function VersionStamp() {
  ...
  return (
    <div className="mt-12 lg:mt-16 print:hidden">
      <p className="text-[0.8125rem] text-foreground/70">
        ...
        <a
          ...
          className={cn(
            adminLink,
            'underline-offset-[3px] hover:text-foreground',
