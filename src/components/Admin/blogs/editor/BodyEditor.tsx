...
import { plainDoc, stripTrailingEmptyParagraphs, type BlogDoc } from '@/lib/blogBody';
import {
  figureBlock,
  filterBlogBlocks,
  instagramBlock,
...
export default function BodyEditor({ postId, doc, onChange, editable = true }: Props) {
  ...
  const editor = useEditor({
    ...
    // The document leaves here CANONICAL AND PLAIN. Canonical, which is what
    // lets the page compare it for equality to decide whether anything is
    // unsaved: `TrailingNode` appends an empty paragraph whenever the last
    // child is not a paragraph, and it does that from `appendTransaction`,
    // which runs on a bare SELECTION change: without the strip, clicking into
    // a post that ends in a figure would count as an edit and fire an autosave
    // nobody asked for. Plain, which is what lets it cross the server-action
    // wire at all: `getJSON()` returns every `attrs` as the null-prototype
    // object prosemirror-model built, React's serializer ships that as a
    // temporary reference, and the server receives a FUNCTION where the zod
    // layer wants an object, so the save door refused every document with a
    // heading in it. `plainDoc` is the JSON round trip that makes it data; its
    // comment carries the whole chain. Doing both here rather than in the page
    // is what keeps `blogBody.ts` (and with it the whole Tiptap schema) inside
    // this async chunk.
    onUpdate: ({ editor: instance }) => {
      onChange(stripTrailingEmptyParagraphs(plainDoc(instance.getJSON() as BlogDoc)));
    },
  });
