...
const ConsentBanner = () => {
  ...
  return (
    <div
      ...
      aria-live="polite"
      className="fixed bottom-0 left-0 right-0 z-100 border-t border-white/10 bg-black text-white shadow-[0_-12px_40px_-8px_rgba(0,0,0,0.4)]"
    >
      <div className="mx-auto flex max-w-7xl flex-col gap-5 px-6 py-5 sm:px-10 md:flex-row md:items-center md:justify-between md:gap-10 md:py-6">
        ...
        <div className="flex flex-row gap-3 md:shrink-0">
          <button
            type="button"
            onClick={deny}
            className="flex-1 border border-white/30 px-5 py-2.5 text-sm font-medium text-white transition hover:border-white hover:bg-white/5 md:flex-none"
          >
            Decline
          </button>
          <button
            type="button"
            onClick={grant}
            className="flex-1 bg-white px-5 py-2.5 text-sm font-medium text-black transition hover:bg-white/90 md:flex-none"
          >
            Accept
          </button>
        </div>
