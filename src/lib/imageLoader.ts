...
// to a pre-generated static AVIF variant — or the native original for excluded marks.
// The optimizer is never involved: these URLs are plain static files served from the
// edge CDN. `quality` is ignored (variants are pre-encoded).
//
// The width buckets must stay aligned with next.config's `deviceSizes: [384, 640, 960, 1280]`
// and `imageSizes: [256]`: 256/384 → -384, 640 → -640, 960 → -960, 1280 → -1280 (encoded
// withoutEnlargement, so it's the native re-encoded when the source is narrower).
