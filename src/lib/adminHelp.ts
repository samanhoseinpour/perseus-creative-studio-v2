...
export const ADMIN_HELP: Record<AdminHelpKey, AdminHelpTopic> = {
  ...
  tasks: {
    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          'The box in that form marks the task you are revising done, and it is ticked to start with \u2014 once there is a round two, round one is finished. It says which month that files it under. Untick it if the original really is still open. A task that is already done never shows the box.',
    ...
    tips: [
      ...
      'To pick several on a computer, move over a row and its tick box appears — the box at the top of that column picks every task on the page, and once anything is picked they all stay visible so you can see what you have. The bar that appears acts on all of them, and “More” on it holds the controls that change a field: member, priority, client, tags and dates.',
    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          '"Show the last 5 minutes" asks Vercel for the request log of the build serving this page and shows what came back: each request with its status code, and beneath it our own log lines with their error class, route and the ids to search by. Any other line is counted and withheld, and a link that carries a credential — a client’s report link, a password-reset link — is shown as its pattern, never the link itself. It is a window — not history, and never a rate — and if there was more than fits, the panel says so.',
  ...
  profile: {
    ...
    tips: [
      ...
      'Notifications say how many things need you, never what they are — no client names, no task titles, no figures. They appear on a lock screen, so the detail stays in the email. You also get one when your account is signed in to, so an unexpected sign-in is visible straight away. Reminders keep arriving on a device whose session has simply lapsed — that is deliberate, since they are most useful when the dashboard is closed — but signing out of a device on purpose stops them there.',
