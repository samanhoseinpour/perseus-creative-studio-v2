...
export const ADMIN_HELP: Record<AdminHelpKey, AdminHelpTopic> = {
  ...
  tasks: {
    ...
    tips: [
      ...
      'Swipe a card left to delete it, or right to mark it done. Both ask before anything happens, and the “done” one is the same window you get on a computer, so you still enter the hours and the day. A task that is already done doesn’t swipe right; reopening it lives in the ⋯ menu, because it would move the task out of a month you may already have reported.',
    ...
    sections: [
      {
        ...
        bullets: [
          ...
          '"Print report" opens the A4 sheet. Use your browser\'s Save as PDF. "CSV" downloads the raw month, one row per task with a "revision" column, so it is where to look when you need the rounds listed separately.',
        ],
      },
    ],
    tips: [
