...
export const ADMIN_HELP: Record<AdminHelpKey, AdminHelpTopic> = {
  overview: {
    ...
    intro:
      'Your starting page. Every card is a shortcut into a section you can open, and nothing here changes any data.',
    sections: [
      {
        heading: 'Your day',
        bullets: [
          ...
          'A row that reads "with" and someone\u2019s name is work you share with them. The hours shown are the task\u2019s whole estimate, not your share of it, the same figure the board shows.',
      ...
      },
      {
        ...
        bullets: [
          ...
          '"Tasks done" on that card counts what was DELIVERED, so three rounds on one video count as one video. Any revision rounds are named beside the number. Their hours are still in the total above.',
          '"Money" adds this month\u2019s salaries, wire fees and bills into one figure, with a bar showing the split. It is the same total the Spend page shows. Only people with both Payroll and Bills access see it.',
          ...
          '"Search anything…" (or ⌘K anywhere) opens the search palette; the links under it lead to every section you can open that has no card of its own, and to your profile. Nothing you have access to is missing from this page.',
        ],
      },
    ],
    tips: [
      'You only see cards for areas you have access to, so the page rearranges itself around them. Access is managed on the Users page.',
    ...
    sections: [
      {
        ...
        bullets: [
          ...
          '"New task" in the header opens the full form instead, with notes, a due date and the deliverable links at the moment you create it, rather than added afterwards.',
          ...
          'Nothing found? If a word looks misspelled you get the results for the corrected spelling with "Showing results for …" above them, and a link back to exactly what you typed.',
          'The headings above the table are buttons. Click one and it offers the orders that column can be read in, then, under a line, the filter for that column. It is the same filter that sits in the bar above, so the two always agree. A small arrow marks whichever column the board is sorted by.',
          '"Tags" and "Member" offer a filter and no order, on purpose: a task can carry several tags and be worked by several people, so filing it under whichever one comes first alphabetically would say something untrue. "Group" is the control for reading the board person by person. "Status" has no filter in its heading either, because the tabs above the table are the status filter.',
          '"Sort" sits beside the search box and always says what the board is ordered by, whether you set it there or from a heading. It is not one of the filters, so "Clear filters" leaves it alone.',
          'A filter that is narrowing the board fills in solid, so one look at the row says what is being applied. "Sort" and "Group" stay outlined however they are set, because they reorder the board rather than narrow it.',
          '"Clear filters" appears at the end of the row, in red with an x, the moment anything is narrowing the board, including a search. It keeps your month, your sort and your grouping.',
          '"Clear sort" appears next to it whenever the board is not in its usual order, and the same row sits inside the "Sort" button and inside the sorting heading\u2019s own menu. The two buttons stay separate on purpose: clearing a client does not throw away how you had the board ordered, and clearing the order does not widen the board back out.',
          'Three buttons in the header change how the same tasks are drawn. "List" is this table. "Calendar" lays the month out day by day. "Digest" shows the last 7 days of completed work. "Export CSV" downloads exactly what the List view shows.',
      'The board shows one month at a time. The month sits in its own row above the tabs, with arrows either side and "All time" in the list for the whole log.',
      'Finished work belongs to the month it was finished in. Work you have not finished is always on the current month, whichever month it started in, so a task can never go missing behind a month boundary.',
      'That is also why a month you have already left shows only "Done", "Delivered", "Posted" and "All": there is no open work filed back there, it is all on this month.',
      'Everything else stays inside the month you are on: the tabs and their counts, search, the filters, sort and "Export CSV". "Clear filters" keeps the month, because the month is not a filter.',
      'Choosing a month in the Digest turns it into that month\u2019s wrap-up: what shipped, how many hours, who worked on it and which kinds of work took the time. Left alone, the Digest stays on the last 7 days, which is usually what you want when a month has only just started.',
      ...
      },
      {
        ...
        bullets: [
          '"Calendar" draws one month as a grid, with each task on a day. Tap a task to open it, exactly as on the board. Nothing is edited on the grid itself.',
          'Which date puts a task on a day is up to you: open the Filters date menu and pick one. "Due or start" is the normal one and gives you a plan of the month ahead. Switch it to "Completed" and the same grid becomes a record of what shipped, day by day.',
          'The heading on each day shows how many tasks it holds and how many hours, then the first few of them. Busy days end in a line like "+43 more", which opens the list filtered to that one day. The number in the heading is always the real total, so a folded day never hides how big it is.',
          ...
          'A red dot means the deadline has passed, an amber one means it is today. Tasks that have already shipped go grey.',
          'A month you have already left is worth opening here. Unlike the List, the calendar will show you work that was due back then and never got finished.',
          'Picking "Completed" leaves only "Done", "Delivered", "Posted" and "All" in the tabs. Work that is not finished has no completion date, so there would be nothing to put on a day.',
          'On a phone the grid becomes a day by day list, showing the same tasks.',
        ],
      },
      {
        heading: 'Revisions',
        bullets: [
          'When work comes back for changes, don\u2019t add a second task for it. Open the \u22ef menu on the original and choose "Add revision". It opens a new task already filled in with the same client, category, tags and title, so all you type is the time it took. It also carries the "Revision" tag for you.',
          'The box in that form marks the task you are revising done, and it is ticked to start with, because once there is a round two, round one is finished. It says which month that files it under. Untick it if the original really is still open. A task that is already done never shows the box.',
          'Rounds stack: adding a revision to a revision points at THAT one, not back at the original. A middle round reads "Revision of" the round before it, and the original counts the whole chain: two rounds on one video read "2 revisions".',
          ...
          'If you start typing a title the studio has already used for that client, the add band offers to log it as a revision instead. "It\u2019s separate" dismisses it, and it never blocks you.',
      ...
      },
      {
        ...
        bullets: [
          ...
          'There is a "Revision" tag and there is also "Add revision" in the \u22ef menu. They are different things, even though "Add revision" now applies the tag for you. The tag is just a label you can filter by; the LINK is what keeps the reports counting one delivery instead of two, and removing the tag never breaks it.',
      ...
      },
      {
        ...
        bullets: [
          ...
          '"Done": the work itself is finished. It now counts in that month\'s client report and on the leaderboard.',
          'Then one of two, never both. "Delivered": the files are in the client\'s hands and they post it themselves. "Posted": we put it live on their channels. Which one depends on whose account it goes on, so a task ends at whichever fits, and neither leads to the other.',
          'Done, Delivered and Posted all count exactly the same. Moving a task on never changes the numbers and never moves it to a different month, so you can keep a task up to date without worrying about a report you already sent.',
          'Marking a task Delivered or Posted asks for the day the client got it. It defaults to today, and it cannot be earlier than the day the work was finished. This day is only ever a record: it does not decide which month the task counts in.',
          'So a task can carry two dates, and the cell shows both: "Done Aug 31 · Posted Sep 3". When they are the same day it says "Done and posted Aug 31" instead of printing the day twice.',
          'Click either date to change it. Changing the day the work was FINISHED can move the task to another month\'s report, and the picker says so when it would. Changing the day the client got it never moves anything.',
        ],
      },
    ],
    tips: [
      ...
      'The date filter\'s "Today" shows tasks dated today (the due date, or the start date when there\'s no deadline), and "today" means today in your own timezone, wherever you are. Open it to filter one specific date instead, like due or completed. It no longer lists months: that is what the month row above the tabs is for.',
      'Adding work while reading a past month? The add band is not there, because a new task is unfinished and would belong to this month. The line in its place takes you back.',
      'On a phone each task is its own card instead of a wide table, so nothing scrolls sideways and a task’s dates and hours stay next to its name. Tap a card to open the task and change anything on it.',
      'Swipe a card left to delete it, or right to move it on: an unfinished task goes to Done, and a done task asks whether it was delivered or posted, because it cannot be both and a swipe cannot say which. Both ask before anything happens, and the window is the same one you get on a computer. A delivered or posted task doesn’t swipe right, because it is finished, and nothing ever swipes backwards; reopening lives in the ⋯ menu, because it would move the task out of a month you may already have reported.',
      'To pick several on a computer, tick the box beside each task; the box at the top of that column picks every task on the page at once. The bar that appears acts on all of them, and “More” on it holds the controls that change a field: member, priority, client, tags and dates.',
      'To pick several on a phone, tap the checkbox on a card or press and hold it. Once anything is picked, tapping a card selects it instead of opening it, and the bar above the list acts on all of them: the same bar, with the same “More”.',
      'Also on a phone: the filters fold behind a "Filters" button and the add band shows just the title until you tap it, so the work itself is what you see when you arrive. The number on the button is how many filters are narrowing the list right now.',
  ...
  leaderboard: {
    ...
    sections: [
      {
        ...
        bullets: [
          ...
          'Rank is the number of things DELIVERED in the window: tasks that have shipped, whether they sit at "Done", "Delivered" or "Posted", counting a task and its revisions as the one thing it is. Hours and "% on time" are context, not part of a score; hours only break an exact tie.',
          'Work shared with someone counts for both of you: each is credited the delivery, and the hours are split evenly, so three hours between two people is an hour and a half each. "Tasks completed" at the top counts each job once and says how many were shared, which is why the rows below can add up to more than it.',
          'Revision rounds show as their own figure beside the hours. They are never folded into the rank, so nobody is ahead for needing more rounds, and nobody\u2019s work is hidden either, since every hour still counts in the hours column.',
    ...
    tips: [
      '"N awaiting sign-off" counts finished tasks still waiting for client approval. They join the ranking once approved to Done, and stay there through Delivered and Posted.',
      ...
      'A task counts in the month it was completed, in your own timezone, so viewers in different countries can see slightly different boards. That\'s expected.',
  ...
  tickets: {
    ...
    tips: [
      ...
      'Status changes send no email, so revisit the list to see movement.',
    ...
    sections: [
      {
        ...
        bullets: [
          ...
          'A report builds itself from the work that shipped that month: hours, deliverables, retainer burn, and who worked on the account. Anything at "Done", "Delivered" or "Posted" counts, and counts the same, so moving a task further along never changes a total.',
          'The delivered-work table shows where each line has got to, and a line above it counts them up ("12 posted · 4 delivered"). A stage with nothing in it is left out rather than printed as a zero. It travels: the client sees it on the PDF and the shared page.',
          'Its "Dates" column says when the work was finished and, where they differ, when the client got it: "Done Aug 31 · Posted Sep 3", or "Done and posted Aug 31" on the same day. Only the first of those decides the month, so a job finished in August and posted in September belongs to August\'s report.',
          '"Before you send this" flags anything still sitting at Done, since nothing on those lines says whether the client has it yet. Marking them changes no total.',
          ...
          'The delivered-work table follows that same rule. A delivery is ONE line carrying the hours of every revision round on it, with a "2 revisions" badge beside the title, so a 4h video with rounds of 45m and 30m reads 5h 15m on one line. The line is dated by its last round, which is the day the client got the finished version, so it can sit further down the table than you expect.',
          'A round whose original went out in an earlier month has nothing to join, so it keeps its own line and says "Revision of" the delivery it belongs to. Its hours count toward the month it was done in, not the month the original shipped.',
          '"Month highlights" is a written note for the client, and it leads the PDF and the shared page. "Set target" stores the retainer hours the report measures against.',
          '"Share" creates a public read-only link for that client and month. "Regenerate" replaces it; "Revoke" kills it instantly.',
          '"Print report" opens the A4 sheet. Use your browser\'s Save as PDF. "CSV" downloads the raw month, one row per task with "stage", "released_on" and "revision" columns, so it is where to look when you need the rounds listed separately.',
        ],
      },
    ],
    tips: [
      ...
      'Share links show live numbers, not a snapshot, so edits after sending change what the client sees.',
  ...
  applications: {
    ...
    intro:
      'Job applications from the careers page: review candidates, open their résumés, and triage. Same flow as Inquiries.',
    sections: [
      {
        heading: 'How it works',
        bullets: [
          ...
          'Each row shows the candidate and the role they applied for, as the role was named when they applied, so the name stays even after that role is deleted from "Careers". Filter by "Role", "Source", or date.',
  ...
  projects: {
    ...
    intro:
      'The manager behind the public /projects portfolio. Changes go live on the site immediately, with no deploy needed.',
    sections: [
      {
        heading: 'How it works',
        bullets: [
    ...
    tips: [
      ...
      'A slug never renames itself on an existing project, because it\'s a live URL.',
    ],
  },
    ...
    sections: [
      {
        ...
        bullets: [
          ...
          'Logo uploads apply immediately, with no Save needed. "Order" decides rail position; lower shows earlier.',
        ],
      },
    ],
    tips: [
      ...
      'Renaming a client changes live site text but does not ping search engines by itself, so tell the owner after a rename.',
    ],
  },
  ...
  careers: {
    ...
    sections: [
      {
        ...
        bullets: [
          ...
          'Three states. "Open" is on the site as "Available", so people can apply and Google sees a job posting. "Filled" stays on the site marked "Position filled", so visitors can see how the team is built, but nobody can apply. "Draft" is only visible here.',
          ...
          'A role can’t be opened without a pay range and a posted date. BC requires the pay range on every public posting, and Google requires the date, so the form refuses the flip until both are in.',
    ...
    tips: [
      'A role’s slug (the link name) is fixed once created, because it is stored on every application for that role. Rename the title freely; the slug stays.',
    ...
    sections: [
      {
        ...
        bullets: [
          'One row per post: its title and address, status, author, category, the keyword it targets, when it was last updated, and when it was or will be published. A "No index" chip means that post asks search engines to leave it out.',
          'Tabs above the list narrow it to one status at a time and each says how many posts it holds. "All" is everything except the trash.',
          ...
          '"New post" creates an empty draft and opens straight into the editor.',
          'Each row\'s menu opens the editor, opens a preview of the draft, opens the live article once it is published, or moves the post to trash. A trashed post\'s menu offers "Restore" instead, or "Delete permanently". Selecting several rows lets you trash or restore them together.',
          '"Authors" and "Categories" in the header manage the people posts are filed under and the sections they sit in.',
        ],
      },
      {
        heading: 'Writing and saving',
        bullets: [
          'What you type is saved on its own about a second and a half after you stop, so there is nothing to press as you work. The word beside the status says where that stands: "Saved", "Saving", "Unsaved changes" or "Not saved".',
          '"Save" does something different: it keeps a numbered copy of the post exactly as it stands, one you can read and come back to. The saving that happens on its own keeps none, so it protects your latest words and leaves no version to return to.',
          'Publishing and scheduling keep a numbered copy too, and either one is also what puts the post out or sets the moment it goes out.',
          'The article fills the middle column, where you write: the title, the hero image with the description and caption that go under it, and the body. Beside it, "Post" holds everything else about the post itself, which is its address, author, category, excerpt, takeaways, FAQs, sources and any related posts. "SEO" holds everything for search engines: the search title and description with a preview of how the post will look in results, an address to point search engines at instead when one is needed, whether they may index the page and follow its links, the image and text used when it is shared, and the keywords it targets. On a narrow screen both sit behind "Settings".',
          'Adding a YouTube video asks whether it is ours, and the answer is not cosmetic. Ticking "This video is on the Perseus channel." tells search engines this post is that video\'s home page, so leave it clear for anybody else\'s upload. It can be changed afterwards on the video block itself, where the same tick box reads "On the Perseus channel".',
          'If somebody else changes the post while you have it open, in another tab or when a scheduled post goes live, saving stops instead of writing over their work and a message at the top of the editor offers to reload. Reloading brings back their version and loses anything you have typed since your last save, so copy out what you need before you press it.',
      ...
      },
      {
        ...
        bullets: [
          ...
          'The helpful vote and the sharing buttons are left off, because a draft is not a published post: a vote would be thrown away, and every share link would lead nowhere.',
        ],
      },
      {
        heading: 'Scheduling',
        bullets: [
          'Instead of publishing right away, set a day and time and the post stays a draft until then. The day and time are on your own clock, and the editor states underneath which calendar day that lands on for the studio, since a time close to midnight can fall on a different day in Vancouver.',
          ...
          'The time can be moved at any point before it fires. Editing the writing of a scheduled post changes only the draft sitting behind it, so set the time again to bring those changes into what goes out, or it publishes with the older words.',
          '"Publish now" sends a scheduled post out without waiting, and "Cancel the schedule" pulls it back to a plain draft.',
        ],
      },
      {
        heading: 'Saved versions',
        bullets: [
          'Every explicit save, publish, schedule, unpublish or restore keeps a numbered copy of the post as it stood at that moment, so nothing already saved is lost by continuing to edit.',
          '"Saved versions" lists them all, and any one of them can be read as a page in the same way a draft can.',
          'Restoring a version brings back its words, its pictures and its SEO fields, and nothing else. The address, the publication dates and the status stay exactly as they are, and nothing reaches the public blog until you publish.',
        ],
      },
      {
        heading: 'What the statuses mean',
        bullets: [
          '"Draft" is where a post starts, and where an unscheduled one goes back to. Nothing outside the dashboard can reach it, and its address can still be changed.',
          '"Scheduled" holds a frozen copy of the post waiting for its moment. It can be sent out early or pulled back to a draft at any time before then.',
          '"Published" is live. From here on the address is fixed, and saving edits changes nothing a reader sees until you press "Update".',
          '"Archived" was live and has been taken down, so its address is a 404. The writing, the saved versions and any votes it collected are all kept, and publishing it again puts back the date it originally went out on.',
          '"Trash" is marked for deletion and holds nothing back: a live post comes off the blog the moment you put it there. Restoring a post that was never published returns it to "Draft"; restoring one that was live returns it to "Archived", so its address stays a 404 until you publish it again.',
        ],
      },
    ],
    tips: [
      ...
      'If you also hold the Activity log area, "History" in the editor opens that post\'s full record of changes.',
    ],
  },
  ...
  feedback: {
    ...
    tips: [
      'One vote per browser per article, and a changed mind overwrites the old vote.',
    ...
    sections: [
      {
        ...
        bullets: [
          ...
          '"How <month> was worked out", at the bottom, shows the working: what landed, your agreed salary, and the rate used when a currency conversion happened.',
        ],
      },
    ],
    tips: [
    ...
    sections: [
      {
        ...
        bullets: [
          ...
          'Members confirm from their own page, or flag a problem, and a flag emails you their note. "Mark received", "Re-sent, clear the flag", "Void" and "Payslip" all live in each row\'s "…" menu.',
    ...
    sections: [
      {
        ...
        bullets: [
          ...
          'The line under the tiles compares the two honestly: salaries and recurring costs set against the run-rate they are committed to. It is a difference, never a sum, and it deliberately leaves out wire fees and one-offs, because neither is something the run-rate ever claimed to cover.',
  ...
  users: {
    ...
    sections: [
      {
        ...
        bullets: [
          ...
          '"No devices" means nobody can be nudged on that account, but it does not say why: they may have turned it off, never been asked, or be on an iPhone that is not on the Home Screen yet. Only they can switch it on, from their own Profile, since a browser will not let anyone do it for them.',
          ...
          'Who can manage whom: the owner\'s row is untouchable; superadmin rows are managed only by the owner; member rows by any superadmin, unless the member holds one of the sensitive chips ("Payroll", "Costs", "Activity log"), and then their reset and delete are owner-only too.',
    ...
    tips: [
      'Access changes apply on the person\'s next navigation, with no sign-out needed.',
    ...
    sections: [
      {
        ...
        bullets: [
          ...
          'Every entry offers "history" for the thing it is about. That is the whole record of that one ticket, project, account or submission, newest first, including what happened after it was deleted. A ticket, an inquiry, an application and a project carry the same "History" link on their own page. "back to everything" returns to the full feed.',
        ],
      },
    ],
    tips: [
      'Salary figures, passwords, message bodies, and applicant contact details never appear here, because they are refused at write time.',
      'Sensitive reads are logged too: opening a résumé or someone\'s payslip writes a "Viewed" row.',
      'Routine task and payroll edits keep their own feeds, and only their structural or destructive acts show here.',
  ...
  monitoring: {
    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          '"Healthy" means every check is passing and nothing is open.',
          '"Degraded" means a warning is open, or a check is failing, timing out or not configured. Something needs a look; the dashboard still works.',
          '"Incident" means a critical incident is open: the database is unreachable, a job failed twice in a row, or one error is repeating fast.',
          '"Unknown" means the checks are stale or a read on this page failed. It is never green by default: if the monitoring job has not run in 35 minutes, the headline says so instead of guessing.',
        ],
      },
      {
      ...
      {
        heading: 'Service levels and recent logs',
        bullets: [
          '"Service levels" scores the last 30 days: for a dependency, the share of checks that passed; for a scheduled job, the share of its scheduled runs that happened and succeeded, where a run that never fired counts against it exactly like one that threw. Each has a target and shows how much of its failure budget is used. "Not enough data" means there is not yet a real sample behind the figure, and it is never rounded up to 100%.',
          '"Requests" is the share of the public site’s responses that were not a server error, from Vercel’s own count of production traffic, folded by the monitoring job every 15 minutes. Only a 5xx counts against it. A 404 or a 401 is a response the server gave as asked. It needs a thousand responses before it shows a figure, and if the count has not been refreshed for two hours the row says when it was last counted instead of reading as current.',
          '"Show the last 5 minutes" asks Vercel for the request log of the build serving this page and shows what came back: each request with its status code, and beneath it our own log lines with their error class, route and the ids to search by. Any other line is counted and withheld, and a link that carries a credential (a client’s report link, a password-reset link) is shown as its pattern, never the link itself. It is a window, not history and never a rate. If there was more than fits, the panel says so.',
          'Both need a Vercel account token scoped to this team in the environment. A project token is refused by both reads, and the panel says so. Until one is set the panel says so and the request row reads "Not enough data". If Vercel does not answer the request-log read within ten seconds, the panel says exactly that rather than reporting a quiet site; the "Runtime logs" link opens the same logs on Vercel.',
        ],
      },
    ],
    tips: [
      ...
      'Latency, request volume and the full request log are measured by Vercel, not by this page; "On Vercel" links straight to them. The request-success figure is the one Vercel number folded in here.',
  ...
  profile: {
    ...
    sections: [
      {
        ...
        bullets: [
          ...
          'Click your avatar to upload or change your photo, and you see the exact preview before anything is saved.',
          '"Password": changing it signs you out everywhere except this device. Resetting a forgotten one from the sign-in page signs you out everywhere, this device included.',
    ...
    tips: [
      'The version number at the bottom of any page opens that same list, wherever you are, so you do not have to come back here. It goes up every time something new ships.',
      ...
      'When a notification arrives, the dashboard\u2019s installed app icon marks itself too: a count on a Mac Dock, a Windows taskbar or an iPhone Home Screen, and a plain dot on Android, where the system draws it rather than us. Either way it clears the moment you open the dashboard. A browser tab has no icon to mark, so this only applies once the dashboard is installed.',
      ...
      'Notifications say how many things need you, never what they are: no client names, no task titles, no figures. They appear on a lock screen, so the detail stays in the email. You also get one when your account is signed in to, so an unexpected sign-in is visible straight away. Reminders keep arriving on a device whose session has simply lapsed (deliberate, since they are most useful when the dashboard is closed), but signing out of a device on purpose stops them there.',
      'Changing or resetting your password ends every signed-in session but does NOT remove passkeys. A passkey is its own credential and never depended on your password, which is why the password box names how many you have. If you are changing it because you think someone else had access, review this list afterwards and remove anything you do not recognise: nobody can ADD a passkey once their session is gone, but one added before that still works.',
