...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (due.length === 0) {
      return Response.json({ day: todayKey, due: 0, created: 0 });
    }
    ...
    if (created.length > 0) {
      try {
        ...
        console.error('[cron] recurring tasks revalidate failed', error);
      }
    }
    ...
    console.error('[cron] recurring tasks failed', error);
